...
int pif_plugin_primitive_hh_finder(EXTRACTED_HEADERS_T *headers, MATCH_DATA_T *match_data)
{
	...
	uint32_t srcAddr, dstAddr, hv0, hv1, hv2;
	...
	srcAddr = PIF_HEADER_GET_ipv4___srcAddr(ipv4_header);
	dstAddr = PIF_HEADER_GET_ipv4___dstAddr(ipv4_header);
	hv0 = hash_func1(srcAddr, dstAddr);
	hv1 = hash_func2(srcAddr, dstAddr);
	hv2 = hash_func3(srcAddr, dstAddr);
	
	
	
	mem_read32(&in_xfer_sketch, &sketch[0][hv0], sizeof(uint32_t));
	out_reg_sketch0 = in_xfer_sketch;
	out_reg_sketch0 += 1;
	out_xfer_sketch = out_reg_sketch0;
	mem_write32(&out_xfer_sketch, &sketch[0][hv0], sizeof(uint32_t));
	
	
	mem_read32(&in_xfer_sketch, &sketch[1][hv1], sizeof(uint32_t));
	out_reg_sketch1 = in_xfer_sketch;
	out_reg_sketch1 += 1;
	out_xfer_sketch = out_reg_sketch1;
	mem_write32(&out_xfer_sketch, &sketch[1][hv1], sizeof(uint32_t));
	
	
	mem_read32(&in_xfer_sketch, &sketch[2][hv2], sizeof(uint32_t));
	out_reg_sketch2 = in_xfer_sketch;
	out_reg_sketch2 += 1;
	out_xfer_sketch = out_reg_sketch2;
	mem_write32(&out_xfer_sketch, &sketch[2][hv2], sizeof(uint32_t));
	...
	//update heavy hitter
	mem_read32(&in_xfer_hh, &heavy_hitter, sizeof(struct Heavy_Hitter));
	...
	if(out_reg_hh.count < out_reg_sketch_min)
	{
		...
		mem_write32(&out_xfer_hh, &heavy_hitter, sizeof(struct Heavy_Hitter));
	}
